		...
		if (!isTemplate) {
			...
			for (int i=0; i<lits.size(); i++) {
				...
				if (!containsExsitentiaVars) {
					...
					attrs.add("array_accum(DISTINCT t" + Integer.toString(i) + ".id) AS \"t" + Integer.toString(i) + ".id\"");
				}
			...
			attrs.clear();
			for (int i=0; i<lits.size(); i++) {
				attrs.add(lits.get(i).getPred().getRelName() + " t" + Integer.toString(i));
			}
			input_query += StringMan.commaList(attrs) + "\n";
			...
			for (int i=0; i<lits.size(); i++) {
				...
				ArrayList<Term> terms = lit.getTerms();
				for (int j=0; j<terms.size(); j++) {
					String var = terms.get(j).var();
					if (!vars.contains(var)) {
						vars.add(var);
						if (!existentialVars.contains(var)) {
							groupBy.add("t" + Integer.toString(i) + "." + lit.getPred().getArgs().get(j));
						}
						for (int k=i; k<lits.size(); k++) {
							Literal lit2 = lits.get(k);
							for (int l=(k==i?j+1:0); l<lit2.getTerms().size(); l++) {
								String var2 = lit2.getTerms().get(l).var();
								if (var2.equals(var)) {
									attrs.add("t" + Integer.toString(i) + "." + lit.getPred().getArgs().get(j) +
										"=t" + Integer.toString(k) + "." + lit2.getPred().getArgs().get(l));
								}
							}
						}
					}
						...
						if (var.startsWith("v")) {
							if (!vars.contains(var)) {
								...
								for (int k=i; k<lits.size(); k++) {
									Literal lit2 = lits.get(k);
									for (int l=(k==i?j+1:0); l<lit2.getTerms().size(); l++) {
										if (lit2.getTerms().get(l).isVariable()) {
											String var2 = lit2.getTerms().get(l).var();
											if (var2.equals(var)) {
												attrs.add("t" + Integer.toString(i) + "." + lit.getPred().getArgs().get(j) +
													"=t" + Integer.toString(k) + "." + lit2.getPred().getArgs().get(l));
							...
							attrs.add("t" + Integer.toString(i) + "." + lit.getPred().getArgs().get(j) +
								"=" + Integer.toString(instance.conList.get(Integer.parseInt(var.substring(1))).constant()));
						}
					}
				}
