...
public class Predicate {
	...
	public void materialize(boolean... specialMode) {
		...
		try {
			...
			String sql = "INSERT INTO " + getRelName() + StringMan.commaListParen(args) + "\n";
			sql += "SELECT ";
			for (int i=0; i<args.size(); i++) {
				attrs.add("t" + Integer.toString(i) + ".constantid");
			}
			sql += StringMan.commaList(attrs) + "\n";
			sql += "FROM ";
			attrs.clear();
			for (int i=0; i<args.size(); i++) {
				attrs.add(types.get(i).getRelName() + " t" + Integer.toString(i));
			}
			sql += StringMan.commaList(attrs) + ";";
			//UIMan.println(sql);
			db.update(sql);

			// mark variables in query file
			for (Atom a : queries) {
				sql = "UPDATE " + getRelName() + "\n";
				sql += "SET inQuery=TRUE";
				attrs.clear();
				for (int i=0; i<a.args.list.length; i++) {
					if (a.args.list[i] >= 0) {
						attrs.add(args.get(i) + "=" + Integer.toString(a.args.list[i]));
					}
				}
				if (attrs.size() == 0) {
					sql += ";";
				} else {
					sql += "\nWHERE " + StringMan.join(" AND ", attrs) + ";";
				}
				db.update(sql);
				//UIMan.println(sql);
			}
			...
			// set soft evidence variables to null
			sql = "UPDATE " + getRelName() + "_evidence\n";
			sql += "SET truth = NULL\n";
			sql += "WHERE club = 0;";
			db.update(sql);

			// copy evidence to variable table
			sql = "UPDATE " + getRelName() + "\n";
			sql += "SET truth=" + getRelName() + "_evidence.truth\n";
			sql += "FROM " + getRelName() + "_evidence\n";
			attrs.clear();
			for (int i=0; i<args.size(); i++) {
				attrs.add(getRelName() + "." + args.get(i) + "=" + getRelName() + "_evidence." + args.get(i));
			}
			sql += "WHERE " + StringMan.join(" AND ", attrs) + ";";
			//UIMan.println(sql);
			db.update(sql);
	...
	public void createEvidenceTable() {
		String relName = this.relName + "_evidence";
		db.dropTable(relName);
		db.dropView(relName);
		db.dropSequence(relName + "_id_seq");
	...
	public ArrayList<String> generateDeepdiveSoftEvidenceInferenceRules() {
		...
		try {
			...
			while(rs.next()) {
				...
				ArrayList<String> attrs = new ArrayList<String>();
				for (String arg : args) {
					attrs.add("t." + arg + "=" + rs.getString(arg));
				}
