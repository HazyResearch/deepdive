...
//       serializier.addVariable(0, false, None, "Boolean", 2, None)
//       serializier.addVariable(1, true, Option(1.0), "Boolean", 1, None)
//       serializier.addVariable(2, false, None, "Multinomial", 0, Option(3))
//       serializier.addFactor(0, 0, "IsTrueFactorFunction", 1)
//       serializier.addFactor(1, 1, "ImplyFactorFunction", 2)
//       serializier.addEdge(0, 0, 0, true, None)
//       serializier.addEdge(0, 1, 0, true, None)
//       serializier.addEdge(1, 1, 1, false, Option(3))
...
//       val variables = List(1,2,3).map(i => FactorGraphProtos.Variable.parseDelimitedFrom(variablesInput))
...
//       assert(variables(0).getIsEvidence === false)
//       assert(variables(1).getId === 1)
//       assert(variables(1).hasInitialValue === true)
//       assert(variables(1).getIsEvidence === true)
//       assert(variables(1).getInitialValue === 1.0)
//       assert(variables(1).getDataType === FactorGraphProtos.Variable.VariableDataType.BOOLEAN)
//       assert(variables(1).getEdgeCount === 1)
//       assert(variables(2).getDataType === FactorGraphProtos.Variable.VariableDataType.MULTINOMIAL)
//       assert(variables(2).getCardinality === 3)
