...
trait SQLInferenceRunner extends InferenceRunner with Logging {
  ...
  // handles variable partition mapping
  def handlePartition(schema: Map[String, _ <: VariableDataType], factorDescs: Seq[FactorDesc], 
    ...
    if (!partitionColumn.isDefined) return
    val groundingDir = getFileNameFromPath(groundingPath)
    ...
    schema.foreach { case(variable, dataType) =>
      ...
      du.unload(InferenceNamespace.getVariablePartitionIdMappingFileName(relation), 
        s"${groundingPath}/${InferenceNamespace.getVariablePartitionIdMappingFileName(relation)}",
        dbSettings, parallelGrounding,
    ...
    factorDescs.zipWithIndex.foreach { case (factorDesc, idx) =>
      ...
      du.unload(InferenceNamespace.getFactorPartitionIdMappingFileName(factorDesc.name), 
        s"${groundingPath}/${InferenceNamespace.getFactorPartitionIdMappingFileName(factorDesc.name)}",
        dbSettings, parallelGrounding,
        s"""SELECT t.id AS id, ${partitionColumn.get} / ${partitionSize} AS pid
        FROM ${querytable} t
        """, groundingDir)
