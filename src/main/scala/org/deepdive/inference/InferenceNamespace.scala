...
object InferenceNamespace {
  ...
  def getVariablePartitionIdMappingFileName(relation: String) = s"dd_partition_variables_${relation}"
  def getFactorPartitionIdMappingFileName(name: String) = s"dd_partition_factors_${name}"
