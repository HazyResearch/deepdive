...
object Pipeline {
  ...
  def run(config: Config) {
    ...
    val extractorExecutor = system.actorOf(ExtracorExecutor.props(Settings.databaseUrl), "ExtractorExecutor")
    Settings.get().extractors.foreach { extractor =>
      extractorExecutor ! ExtracorExecutor.Execute(ExtractionTask(extractor.outputRelation, extractor.inputQuery, extractor.udf))
    }
