...
Word = namedtuple('Word', [
  ...
  'dep_par',
  'dep_label'])
Span = namedtuple('Span', ['begin_word_id', 'length'])
Sequence = namedtuple('Sequence', ['is_inversed', 'elements'])
DepEdge = namedtuple('DepEdge', ['word1', 'word2', 'label', 'is_bottom_up'])

def unpack_words(input_dict, char_offset_begin=None, char_offset_end=None, lemma=None, \
  ...
  array_char_offset_begin = input_dict[char_offset_begin] if char_offset_begin != None else ()
  array_char_offset_end = input_dict[char_offset_end] if char_offset_end != None else ()
  ...
  ziped_tags = map(None, array_char_offset_begin, array_char_offset_end, array_lemma, \
  ...
  for i in range(0,len(ziped_tags)):
    ...
    wordobjs.append(Word(
      begin_char_offset=ziped_tags[i][0],
      end_char_offset=ziped_tags[i][1],
      lemma=ziped_tags[i][2],
      pos=ziped_tags[i][3],
      ner=ziped_tags[i][4],
      word=ziped_tags[i][5],
      dep_par=dep_tree[i]["parent"],
      dep_label=dep_tree[i]["label"]))
  return wordobjs
...
def _seq_between_words(words, begin_idx, end_idx, func=lambda x:x):
...
def tokens_between_spans(words, span1, span2, f=lambda x:x):
  """
  ...
  if span1.begin_word_id < span2.begin_word_id:
    return _seq_between_words(words, span1.begin_word_id+span1.length-1, span2.begin_word_id, f)
  else:
    return _seq_between_words(words, span1.begin_word_id, span2.begin_word_id+span2.length-1, f)
...
def _path_to_root(words, word_idx):
  ...
  covered_indexes = set()
  while True:
    if c_word_idx in covered_indexes: break
    rs.append(words[c_word_idx])
    covered_indexes.add(c_word_idx)
    if words[c_word_idx].dep_par == -1 or words[c_word_idx].dep_par == c_word_idx: break
...
def dep_path_between_words(words, begin_idx, end_idx):
  ...
  for word in path_to_root1:
    ...
    path.append(
      DepEdge(word1=word, word2=words[word.dep_par], label=word.dep_label, is_bottom_up=True))
  path_right = []
  for word in path_to_root2:
    if word in common: break
    path_right.append(
      DepEdge(word1=words[word.dep_par], word2=word, label=word.dep_label, is_bottom_up=False))
