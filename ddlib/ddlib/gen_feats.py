...
def _get_substring_indices(_len, max_substring_len):
  ...
  for start in range(_len):
    for end in reversed(range(start + 1, min(_len, start + 1 + max_substring_len))):
      yield (start, end)
  ...
  for i in range(span.begin_word_id, span.begin_word_id + span.length):
    for j in range(1, window + 1):
      if i+j <= span.begin_word_id + span.length:
        yield "NGRAM_" + str(j) + "_[" + \
          " ".join(map(lambda x: str(x.lemma), sentence[i:i+j])) + "]"
...
def _get_window_features(sentence, span, window=3, combinations=True, isolated=True):
  """
...
def _get_dictionary_indicator_features(sentence, span, window=3, prefix="IN_DICT"):
  """
...
def get_sentence(begin_char_offsets, end_char_offsets, words, lemmas, poses, \
  ...
  word_obj_list = unpack_words(
    obj,
    char_offset_begin='ch_of_beg',
    char_offset_end='ch_of_end',
    lemma='lemma',
    pos='pos',
    ner='ner',
    words='words',
