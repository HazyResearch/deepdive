...
[feature extraction](overview.md#extractors). This document presents the
different types of extractors supported by DeepDive. Please refer to the
[Configuration reference](configuration.md#extractors) for a more structured
...
For example, in our [walkthrough](walkthrough.md), to debug the extractor `ext_has_spouse_features`, just change
`deepdive.conf` to:
