...
graphs](inference.html), [relation
extraction](relation_extraction.html), and [distant
supervision](distant_supervision.html). It describes each step
...
the data to extract [entities](relation_extraction.html#entity), perform
entity linking, feature extraction, [distant
supervision](distant_supervision.html), and any other task
necessary to create the variables on which it will then perform
[inference](inference.html), and, if needed, to generate the training
...
DeepDive uses a [factor graph](inference.html) to perform
...
supervision](distant_supervision.html) or specified by the user while
...
inference](inference.html#marginal) on the factor graph variables to
learn the probabilities of different values they can take over all [possible
worlds](inference.html#possibleworlds). DeepDive uses our
[high-throughput DimmWitted sampler](sampler.html) to perform [Gibbs
sampling](inference.html#gibbs), i.e., to go through many possible
