...
2. Select `deepdive` or `deepdive_from_release` when asked.
    ...
    * For installation with `deepdive_from_source` option, extra build dependencies are installed, and DeepDive source tree is cloned at `./deepdive`, then executables are installed under `~/local/bin/`.
    ...
    See the [developer's guide](developer.html#build-test) for more details.
...
Congratulations! DeepDive is now installed on your system, and you can proceed to the [next steps](walkthrough.html).
...
If you still want to build DeepDive from source code yourself, follow the [developer's guide](developer.html#build-test).
