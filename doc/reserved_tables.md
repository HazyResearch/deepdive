...
- `dd_graph_variables_holdout`: a table that contains all variable ids that are used for holdout. Can be used for custom holdout by a [holdout query](calibration.html#custom_holdout).

- `dd_graph_variables_observation`: a table that contains all variable ids that are evidence that will not be fitted during learning. An usage example of this table can be found [here](configuration.html#calibration).
...
- `dd_query_[RULE_NAME]`: a table that is defined by the input query of an [inference rule](inference_rules.html). You can use it as a feature table in BrainDump.
