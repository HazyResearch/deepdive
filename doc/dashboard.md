...
To bring up a GUI for the dashboard, simply run:

```bash
mindbender dashboard
```

Let's observe that running a search GUI for [browsing data](browsing.md) will also let you see the dashboard (using the dropdown menu on the top-left).

### Report Templates

Report templates, accessible from the "Configure Templates" page (in the GUI), define the data displayed in a report, as well as the appearance of the report. A number of general report templates come with Dashboard, but you may also [create your own](#writing-custom-templates).
...
More info on nested templates in the advanded [section](#extending-report-templates)
